...
class SeasonFilter extends Component {
  ...
  render() {
    ...
    const springFilter = data.filter(object => object.season === "spring");
    const summerFilter = data.filter(object => object.season === "summer");
    const fallFilter = data.filter(object => object.season === "fall");
    const winterFilter = data.filter(object => object.season === "winter");

    const seasonFilter =
      this.props.selectedSeason.value === "spring"
        ? springFilter
        : this.props.selectedSeason.value === "summer"
        ? summerFilter
        : this.props.selectedSeason.value === "fall"
        ? fallFilter
        : this.props.selectedSeason.value === "winter"
        ? winterFilter
        : null;

    return (
      <React.Fragment>
        {seasonFilter.map((product, id) => (
          <div
